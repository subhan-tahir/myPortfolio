...
import pdf from "../Frontend-Developer.pdf";
    ...
    <>
      <div className={` bg-black shadow-xl  w-full flex justify-between items-center text-white md:px-4 px-2 py-6 relative `}>
        <Container className={"flex justify-between items-center !px-0 md:gap-3"}>
          ...
          <div className="flex-1">
            <ul className="sm:gap-6 sm:text-[17px] font-[600] font-sans md:flex hidden justify-center items-center">
              {menu.map((item, index) => (
                <li className="cursor-pointer  group relative" key={index}>
                  <a href={item.link} className="">
                    <motion.div initial={{opacity:0,scaleY:0}} animate={{opacity:1,scaleY:1}} transition={{duration:0.8,delay:index*0.2}}>
          ...
          <div className="flex items-center md:gap-4 gap-2 flex-1 justify-end">
            ...
            <div
              className="burgerButton md:hidden flex text-white bg-transparent text-2xl font-extralight cursor-pointer"
              onClick={handleShow}
            >
              {icon ? (
                <button className="relative h-10 w-10 rounded-full bg-black transition-colors hover:bg-black">
                  <span
